...
int main()
{
    // FIXME: I'm too lazy to fix, but for some reason debugging causes the cwd to change to the root repo
    string cwd = std::filesystem::current_path();
    if (!hasEnding(cwd, "LCL_20_50"))
    {
        std::filesystem::current_path(cwd + "/CPPCodeBase/LCL_20_50");
    }
