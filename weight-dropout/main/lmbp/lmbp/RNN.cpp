...
//     colvec idq_ref;
//     int seed = ((trajectory_number + 1) % 10) * 10000 + change_number;
//     // Set the random seed for mlpack (and thereby Armadillo)
//     mlpack::RandomSeed(seed);
//     // Now generate the random values using Armadillo's randu()
//     idq_ref = colvec({ -300, -300 }) + colvec({ 600, 360 }) % mat(2, 1, arma::fill::randu);
//     idq_ref = arma::round(idq_ref * 10) / 10;
//     util::save_matrix_to_csv(idq_ref, "idq_ref_random.csv");
//     return idq_ref;
// }
...
void RNN::find_smallest_weight(const mat& W, const vector<pair<int, int>>& candidates, double& smallest, pair<int, int>& index)
{
    smallest = abs(W(candidates[0].first, candidates[0].second));
    index = candidates[0];
...
void RNN::remove_smallest_weight(mat& W, pair<int, int> index, vector<pair<int, int>>& dropped, vector<pair<int, int>>& candidates)
{
    ...
    [index](const std::pair<int, int>& candidate) {
        return candidate.first == index.first && candidate.second == index.second;
    }), candidates.end());
...
void RNN::train_best_weights(const int max_iterations, bool verbose)
{
	...
	for (int iteration = 1; iteration < max_iterations + 1; iteration++)
	{
        ...
        arma::mat idq_his, idq_ref_his;
        // Initialize variables
        arma::rowvec hist_err_subtotal = arma::zeros<arma::rowvec>(_trajectory_length * samples_to_process);
        arma::mat j_matrix_subtotal = arma::zeros<arma::mat>(_trajectory_length * samples_to_process, _num_weights);
        double j_total_subsum = 0.0;
		...
		if (_rank == 0)
		{
			...
			while (mu < _mu_max)
			{
                ...
                //success = util::cholesky_arma(L, R, h_matrix);
                    //cout << " Here is the value of success and the next value is mu_max "<< success, _mu_max, mu ;  // checking i need to use Armadillo instead of Eigen
				...
				if (mu == _mu_max)
				{
                    cout << "Cholesky decomp failedddddddd!!!" << endl;
					break;
				}
        ...
        if (current_drop < _max_drop) {
            std::cout << "Removing weight when max_drop is " << _max_drop << std::endl;
