...
void recv_matrices(vector<mat*> matrices, int sender)
{
    ...
    int total_count=count_in_matrices(matrices);
    double *payload = new double[total_count];
    MPI_Recv(payload, total_count, MPI_DOUBLE, sender, MPI_PAYLOAD_TAG, MPI_COMM_WORLD, NULL);
...
int main()
{
    ...
    for (int num_sample_time = 6; num_sample_time <=10; num_sample_time++)
    {
        if (rank == 0)
        {
        ...
        // We want to make sure all workers wait here until all are ready so that when
        // we change num_samples, inactive workers may have something to do
                ...
                for (int iteration = 1; iteration < num_iterations + 1; iteration++)
                {
                    ...
                    if (rank == 0)
                    {
                        ...
                        for (int i=1; i < active_workers; i++)
                        {
                            ...
                            MPI_Send(&j_total_sum, 1, MPI_DOUBLE, i, MPI_J_TOTAL_TAG, MPI_COMM_WORLD);
                    ...
                    else
                    {
                        ...
                        MPI_Recv(&j_total_sum, 1, MPI_DOUBLE, 0, MPI_J_TOTAL_TAG, MPI_COMM_WORLD, NULL);
                        recv_matrices({&hist_err_total, &j_matrix_total}, 0);
                    }
                    ...
                    while (mu < mu_max)
                    {
                        ...
                        if (rank == 0)
                        {
                            ...
                            for (int i=1; i < active_workers; i++)
                            {
                                MPI_Send(&j_total_sum2, 1, MPI_DOUBLE, i, MPI_J_TOTAL_TAG, MPI_COMM_WORLD);
                            }
                        ...
                        else
                        {
                            ...
                            MPI_Recv(&j_total_sum2, 1, MPI_DOUBLE, 0, MPI_J_TOTAL_TAG, MPI_COMM_WORLD, NULL);
                            //cout << "Sending subsum2: " << j_total_subsum2 << ", rank: " << rank << endl;
                            //cout << "Sending totalsum2: " << j_total_sum2 << ", rank: " << rank << endl;
                        }

                        //cout << "rank: " << rank << ", j_total_sum: " << j_total_sum << ", j_total_sum2: " << j_total_sum2 << endl;
